...
| 命令 | 说明 |
|---|---|
| `/mh 怪物 <名字>` | 一条消息给出：怪物名 → 弱点属性（各属性取吸收最高的 5 行求平均，展示前二）→ 肉质表 → 异常累积 |

**技能**（永远发文本，不转图）

| 命令 | 说明 |
|---|---|
| `/mh 技能 <名字>` | 技能各等级效果（中文描述） |

**其他**

| 命令 | 说明 |
|---|---|
| `/mh 作品` | 列出已启用的作品（标注当前作品） |
| `/mh 作品 <作品名>` | **仅管理员**：切换当前作品，所有查询随之切换 |
| `/mh 更新` | 管理员: 在线刷新当前作品的数据 |
| `/mh 帮助` | 显示完整帮助 |
...
v0.3.8 起各命令**不再接受 `[作品]` 后缀**：全局只有一份「当前作品」，存放在配置项
`default_game` 里，所有查询都用它。管理员用 `/mh 作品 <作品名>` 切换（例如
`/mh 作品 荒野`），会**同步写回配置**，重启后仍生效；`/mh 作品`（不带参数）则照旧
列出可用作品并标注当前作品。切换是**管理员专属**，帮助与指令列表里不出现这个用法。

作品标识：`mhworld / world / 世界 / iceborne`、`mhrise / rise / 崛起 / sunbreak`、
`mhwilds / wilds / 荒野`。指令组本身也有别名 `/怪物猎人`。
